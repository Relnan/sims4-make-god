...
def _get_dump_filepath(targets, prefix="rmg_dump"):
    """Generiert den Dateinamen. Bei mehreren Zielen wird es ein Household-Dump."""
    ...
    if len(targets) > 1:
        ...
        # Einzel-Sim Dump
        sim = targets[0]
        gender_str = str(sim.gender).split('.')[-1].lower()
        occult_str = mg_utils.get_occult_type(sim)
        return os.path.join(mg_config.MOD_FOLDER, f"{prefix}_{gender_str}_{occult_str}_{sim.first_name}_{sim.last_name}_{timestamp}.md")
    else:
        return os.path.join(mg_config.MOD_FOLDER, f"{prefix}_Empty_{timestamp}.md")

def _get_sim_data_md(sim_info):
    """Sammelt alle Daten eines einzelnen Sims und formatiert sie als Markdown-String."""
    md_lines = []
    
    # Lade Blacklist-Filter aus der Config (oder nutze Standardwerte gegen Junk)
    dump_blacklist = [b.lower() for b in mg_config.get("dump_blacklist_keywords", ["_error", "_high", "_low", "caspartid", "index_"])]
    
    # --- HEADER ---
    md_lines.append(f"# Sim: {sim_info.first_name} {sim_info.last_name}")
    
    try:
        age_str = str(sim_info.age).split('.')[-1]
        gender_str = str(sim_info.gender).split('.')[-1]
        occult_str = mg_utils.get_occult_type(sim_info)
        md_lines.append(f"**ID:** `{sim_info.sim_id}` | **Alter:** {age_str} | **Geschlecht:** {gender_str} | **Okkult:** {occult_str}")
    except:
        md_lines.append(f"**ID:** `{sim_info.sim_id}` (Fehler beim Auslesen weiterer Basisdaten)")
    ...
    if hasattr(sim_info, 'trait_tracker') and sim_info.trait_tracker:
        ...
        if t_list:
            ...
            md_lines.append("- *Keine Traits gefunden.*")
    else:
        md_lines.append("- *Trait-Tracker nicht verfügbar.*")

    # --- PERKS & BUCKS (Occult Perks, Ruhm etc.) ---
    md_lines.append("\n## 🌟 Perks & Bucks (Okkult & Ruhm)")
    if hasattr(sim_info, 'bucks_tracker') and sim_info.bucks_tracker:
        perks_list = []
        try:
            # Versuche verschiedene bekannte EA-Methoden für freigeschaltete Perks
            unlocked = []
            if hasattr(sim_info.bucks_tracker, 'get_unlocked_perks'):
                unlocked = sim_info.bucks_tracker.get_unlocked_perks()
            elif hasattr(sim_info.bucks_tracker, '_unlocked_perks'):
                unlocked = sim_info.bucks_tracker._unlocked_perks.keys()
        ...
        perks_list.sort()
        if perks_list:
            md_lines.extend(perks_list)
        else:
            md_lines.append("- *Keine aktiven Perks gefunden (oder Tracker leer).*")
    else:
        md_lines.append("- *Bucks-Tracker nicht verfügbar.*")

    # --- SPELLS & UNLOCKS (Zauber, Tränke) ---
    md_lines.append("\n## 🔮 Zaubersprüche & Unlocks")
    spell_list = []
    if hasattr(sim_info, 'unlock_tracker') and sim_info.unlock_tracker:
        try:
            for unlock in sim_info.unlock_tracker.get_unlocked_items_gen():
                u_name = getattr(unlock, '__name__', str(unlock))
                # Wir filtern nach bekannten Magie/Zauber-Keywords, um die Liste sauber zu halten
    ...
    spell_list.sort()
    if spell_list:
        md_lines.extend(spell_list)
    else:
        md_lines.append("- *Keine relevanten Zauber/Tränke im Unlock-Tracker gefunden.*")
    ...
    if skill_manager:
        ...
        for stat_type in tuple(skill_manager.types.values()):
            if hasattr(stat_type, 'is_skill') and stat_type.is_skill:
                ...
                # Nur auflisten, wenn der Sim den Skill hat (>0)
                if current_val > 0:
                    s_list.append(f"- **[Level {current_val:02d}]** {s_name}")
        s_list.sort()
        if s_list:
            md_lines.extend(s_list)
        else:
            md_lines.append("- *Keine aktiven Skills gefunden.*")
    else:
    ...
    if tracker and skill_manager:
        f_track = skill_manager.get(16650) # Freundschaft
        r_track = skill_manager.get(16651) # Romantik
        r_list = []
        
        for target_id in tracker.target_sim_gen():
            target_sim = mg_utils.get_sim_by_id(target_id)
            if target_sim:
                t_name = f"{target_sim.first_name} {target_sim.last_name}"
            else:
                t_name = f"Unbekannter Sim"

            bits = tracker.get_all_bits(target_id)
            ...
            # Formatiere den Block für diesen Ziel-Sim
        ...
        if r_list:
            md_lines.extend(r_list)
        else:
            md_lines.append("- *Keine Beziehungen gefunden.*")
    else:
    ...
    stats_list.sort()
    if stats_list:
        md_lines.extend(stats_list)
    else:
        md_lines.append("- *Keine Stats gefunden.*")

    md_lines.append("\n<br>\n") # Abstand zum nächsten Sim im Dokument
    return "\n".join(md_lines)
...
def execute_dump_to_file(targets, out):
    ...
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Anzahl exportierter Sims:** {len(targets)}\n\n")
...
def execute_reference_dump(out):
    ...
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("Nutze diese exakten Namen zum Kopieren in deine `make_god_config.json`.\n\n")
            ...
            # Suche in Snippets (wo einige Zauber liegen)
            if snippet_manager:
...
def cmd_rmg_dump(*args, _connection=None):
    ...
    # NEU: Abfangen des Reference Dumps
    ...
    # Aufruf der neuen Sammel-Funktion
